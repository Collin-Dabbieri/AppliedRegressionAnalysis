...
agx =crossFactors(~Attend*Gender, data = course.df)
summaryStats(Exam~agx, data = course.df)
dev.new(noRStudioGD = TRUE)
interactionPlots(Exam ~ Attend+Gender, data = course.df)
interactionPlots(Exam ~ Gender+Attend, data = course.df)


# Do something similar with  arousal.df
data(package="s20x")

gp.lm = lm(arousal ~ gender + picture + gender:picture,data=arousal.df)
g.lm = lm(arousal ~ gender ,data=arousal.df)
dev.new(noRStudioGD = TRUE)
summary1way(g.lm)
summary2way(gp.lm, page = "interaction")
